...
namespace net.minecraft.src
{
	public class MapGenCavesHell : MapGenBase
	{
		...
		/// </summary>
		protected virtual void GenerateLargeCaveNode(int par1, int par2, byte[] par3ArrayOfByte, double par4, double par6, double par8)
		{
		...
		/// </summary>
		protected virtual void GenerateCaveNode(int par1, int par2, byte[] par3ArrayOfByte, double par4, double par6, double par8, float par10, float par11, float par12, int par13, int par14, double par15)
		{
			double d = par1 * 16 + 8;
			double d1 = par2 * 16 + 8;
			float f = 0.0F;
			...
			int j = random.Next(par14 / 2) + par14 / 4;
			bool flag1 = random.Next(6) == 0;

			for (; par13 < par14; par13++)
			{
				double d2 = 1.5D + (double)(MathHelper2.Sin(((float)par13 * (float)Math.PI) / (float)par14) * par10 * 1.0F);
				double d3 = d2 * par15;
				float f2 = MathHelper2.Cos(par12);
				float f3 = MathHelper2.Sin(par12);
				par4 += MathHelper2.Cos(par11) * f2;
				par6 += f3;
				par8 += MathHelper2.Sin(par11) * f2;
				...
				d4 = MathHelper2.Floor_double(par4 - d2) - par1 * 16 - 1;
				int k = (MathHelper2.Floor_double(par4 + d2) - par1 * 16) + 1;
				d5 = MathHelper2.Floor_double(par6 - d3) - 1;
				int l = MathHelper2.Floor_double(par6 + d3) + 1;
				d6 = MathHelper2.Floor_double(par8 - d2) - par2 * 16 - 1;
				int i1 = (MathHelper2.Floor_double(par8 + d2) - par2 * 16) + 1;
		...
		{
			int i = Rand.Next(Rand.Next(Rand.Next(10) + 1) + 1);
